...
# And even if a name clash happens, admins will notice it during approval
  return (db_numrows(db_execute("SELECT group_id FROM groups WHERE unix_group_name LIKE ? AND status <> 'I'",
				array($form_unix_name))) == 0);
}


session_require(array('isloggedin'=>'1'));
$HTML->header(array('title' => sprintf(_("%s hosting request"), $GLOBALS['sys_name'])));
...
$form->addElement('text', 'unix_name', _('Short/system name') . '<br /><span style="font-size: smaller">'
  . _('(used in URLs, mailing lists names, etc.)') . '</span>');

$form->addElement('header', 'title_information', _('Project information'));
$form->addElement('textarea', 'purpose', _('~20-lines technical description') . '<br /><span style="font-size: smaller">'
...
$form->addElement('checkbox', 'cl_requirements', _('I agree with the <a href="requirements.php">hosting requirements</a>'));

$form->addElement('header', 'title_details', _('Details'));
$form->addElement('textarea', 'required_sw', _('Dependencies') . '<br /><span style="font-size: smaller">'
  . _('name + license + website for each dependency') . '</span>');
$form->addElement('textarea', 'comments', _('Other Comments'));
$form->addElement('text', 'tarball_url', _('Tarball (.tar.gz) URL') . '<br /><span style="font-size: smaller">'
  . _('(Need a place to upload? <a href="upload.php" target="_blank">Open upload window</a>)') . '</span>');
$form->addElement('submit', null, _("Register project"));
  
$form->addRule('full_name', _("Invalid full name"), 'minlength', 2);
$form->addRule('unix_name', _("Invalid Unix name"), 'callback', 'account_groupnamevalid');
$form->addRule('unix_name', _("A project with that name already exists."), 'callback', 'project_does_not_already_exist');
...
$form->addRule("cl_requirements", _("Please accept the hosting requirements"), 'required');

$form->addRule('purpose', _("This is too short!"), 'minlength', 30);
$form->addRule('tarball_url', _("Please give us a link to your project latest release"), 'minlength', 4);
